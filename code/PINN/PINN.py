...
class SquareLoss:
    ...
    def __init__(self, equations, equations_data, args, gamma: float, combined_grad_pre) -> None:
        """
        Args:
            equations:
        ...
        self.gamma = gamma
        self.args = args
        self.combined_grad_pre = combined_grad_pre  # 保存 combined_grad


    def __call__(self, X_eqn, data_pts, weights, grad_weights, net) -> Dict[str, tf.Tensor]:
        y = X_eqn[..., 3:4]
        x = X_eqn[..., 2:3]
        t = X_eqn[..., 0:1]
        z = X_eqn[..., 1:2]
        equations = self.eqns(t=t, z=z, x=x, y=y, args=self.args, neural_net=net)
        ...
        datas = self.eqns_data(t=t_data, z=z_data, x=x_data, y=y_data, Y=Y_data, combined_grad_pre = self.combined_grad_pre, neural_net=net)

        loss_e = sum(tf.reduce_mean(tf.square(eqn)) for eqn in equations)
        loss_d = tf.reduce_mean(weights*(tf.square(datas[0])))+tf.reduce_mean(weights*(tf.square(datas[1])))+tf.reduce_mean(weights*(tf.square(datas[2])))\
                 +tf.reduce_mean(0.1*grad_weights*(tf.square(datas[3])))+tf.reduce_mean(0.1*grad_weights*(tf.square(datas[4])))+tf.reduce_mean(0.1*grad_weights*(tf.square(datas[5])))\
                 +tf.reduce_mean(0.1*grad_weights*(tf.square(datas[6])))+tf.reduce_mean(0.1*grad_weights*(tf.square(datas[7])))+tf.reduce_mean(0.1*grad_weights*(tf.square(datas[8])))\
                 +tf.reduce_mean(tf.square(datas[9]))
...
class OptimizerBase(ABC):
    ...
    def __init__(self, net, loss, collocation_points, data_points, weights, grad_weights) -> None:
        """
        Args:
        ...
        #   记录collocation points会不会变化
        self.initial_colloc_pts = np.copy(collocation_points)
    ...
    # 记录collocation points
    def check_and_record_collocation_points_change(self):
        # 实时监测collocation_points的变化并记录下来
        if not np.array_equal(self.colloc_pts, self.initial_colloc_pts):
            print("Collocation points have changed.")
        else:
            print("Collocation points have not changed.")
...
class LBFGS(OptimizerBase):
    ...
    def _single_iteration(self, params, partition_indices, stitch_indices):
        ...
        with tf.GradientTape() as tape:
            ...
            losses = self.loss(X_eqn=self.colloc_pts, data_pts=self.data_pts, weights=self.weights, grad_weights = self.grad_weights, net=self.net)
    ...
    def optimize(self, nIter: int):
        ...
        self.net.save(f'./my_model/model_final_lbfgs.h5')
...
class Adam(OptimizerBase):
    ...
    def __init__(self, net, loss, collocation_points, data_points, weights, grad_weights) -> None:
        super().__init__(net, loss, collocation_points, data_points, weights, grad_weights)
        ...
        for it in range(nIter):
            ...
            if it % 1000 == 0:
                ...
                for name, loss in losses.items():
                    ...
                    msg += f"{name}: {loss_val:4e}; "
                print(msg)
                self.net.save(f'./my_model/my_model_checkpoint_adam_{it}.h5')
                self.check_and_record_collocation_points_change()
